...
class API():
    ...
    def targetinfo(self):
        ...
        self.index = self.target.index[0]

        if self.index < len(bus_info) - 1:
            self.front = bus_info.iloc[self.index+1]
            self.front = pd.DataFrame(self.front).T
        else: self.front = []
        if self.index > 0:
            self.back = bus_info.iloc[self.index-1]
